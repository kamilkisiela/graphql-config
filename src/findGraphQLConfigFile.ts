...
export function findGraphQLConfigFile(filePath: string): string {
  ...
  const subDirectories = readdirSync(filePath)
    .map(dir => join(filePath, dir))
    .filter(dir => {
      return lstatSync(dir).isDirectory();
    });
  const subDirectoriesWithGraphQLConfig = subDirectories
    .map(subDirectory => {
      const subDirectoryFiles = readdirSync(subDirectory)
        .filter(file => {
          return !lstatSync(join(subDirectory, file)).isDirectory();
        })
        .map(file => {
          return basename(join(subDirectory, file));
        });
